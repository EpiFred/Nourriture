...
{
    if (field === undefined)
        return ({request: "error", code: errorCodes.recipe.missingField, message: "The field '" + type + "' is mandatory and has not been specified."});
    if (field == "")
        return ({request: "error", code: errorCodes.recipe.invalidField, message: "The field '" + type + "' is invalid"});
...
function CheckIngredientListIntegrity(ingredientList)
{
    for (var i = 0; i < ingredientList.length; i++)
    {
        if (!(ingredientList[i].hasOwnProperty("id")))
            return ({request: "error", code:  errorCodes.recipe.invalidFoodsList, message: "List of foods is invalid. The field 'id' is not set."});
        if (!(ingredientList[i].hasOwnProperty("name")))
            return ({request: "error", code:  errorCodes.recipe.invalidFoodsList, message: "List of foods is invalid. The field 'name' is not set."});
        if (!(ingredientList[i].hasOwnProperty("detail")))
            return ({request: "error", code:  errorCodes.recipe.invalidFoodsList, message: "List of foods is invalid. The field 'detail' is not set."});
        for (var _obj in ingredientList[i])
        {
            if (_obj != "id" && _obj != "name" && _obj != "detail")
                return ({request: "error", code:  errorCodes.recipe.invalidFoodsList, message: "List of foods is invalid. The field '"+ _obj +"'is not asked."});
            if ((_obj == "id" && ingredientList[i][_obj] == "") || (_obj == "name" && ingredientList[i][_obj] == "") || (_obj == "detail" && ingredientList[i][_obj] == ""))
                return ({request: "error", code:  errorCodes.recipe.invalidFoodsList, message: "List of foods is invalid. The field '"+ _obj +"'is empty."});
        }
    }
...
function CheckIngredientsList(ingredientList, req, res, next)
{
    ...
    if (ingredientList === undefined)
        ...
        return (res.status(400).send({request: "error", code: errorCodes.recipe.createNoFood, message: "The field 'foods' can't be empty."}));
                ...
                (function(i) {
                    food_collection.findOne({_id: new BSON.ObjectID(ingredientList[i].id)}, function(err_find, food_found)
                    {
                        if (err_find)
                            ...
                            return (res.status(404).send({request: "error", code: errorCodes.recipe.foodDoesNotExist, message: "The id of the food '"+ ingredientList[i].name +"' does not exist."}));
                        else
                        if (ingredientList[i].name != food_found.name)
                            return (res.status(400).send({request: "error", code: errorCodes.recipe.foodsNameDoesNotMatch, message: "The 'name' of the food '"+ ingredientList[i].name +"' does not match with the name in db '"+ food_found.name +"'."}));
                    });
                })(i);
            }
            next();
